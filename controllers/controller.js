...
let postHome = async (req, res) => {
    ...
    try {
        ...
        let { name, phone, email, dob, address, prev_education, stream, course_instrested, entrance_marks } = data

        // check if any field is missing

        if (!name || !phone || !email || !dob || !address || !prev_education || !stream || !course_instrested || !entrance_marks) {
            console.log("not a valid data i.e. a field might be missing !")
            ...
            if (!checkDuplicate) {
                ...
                setStatusMessage("duplicate entry please change email / phone !")
                res.status(400).redirect("/")
            }
